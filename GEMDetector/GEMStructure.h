...
public:
	...
	void addAPV(std::string line){
		...
		if (result.size() == 14) {
			...
			int layerModule = GEM::GetUID(0,layer,dimension,GEMPOS);
			
			layerModuleMap[layerModule].push_back(apvUniqueID);
  ...
  std::map<int, std::vector<int>> GetLayerModuleMap(){
    return layerModuleMap;
  }
...
private:
  ...
  std::map<int, std::vector<int>> layerModuleMap; //(layer , module) -> vector of apvs
